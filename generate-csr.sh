...
if [ -d "$domain" ]; then
	...
	openssl genrsa -aes256 -out $keyPath -passout pass:$password 2048 &>> $logPath
	echo " Done"

	echo -n "Removing passphrase from private key"
	cp $keyPath $keyorgPath > /dev/null
	openssl rsa -in $keyorgPath -out $keyPath -passin pass:$password &>> $logPath
	echo " Done"
	...
	openssl req -new -key $keyPath -out $csrPath -subj "$subject" &>> $logPath
	echo " Done"

	echo -n "Generating encrypted zip"
	7za a -tzip -p"$password" -mem=AES256 $zipPath $keyPath &>> $logPath
	echo " Done"
